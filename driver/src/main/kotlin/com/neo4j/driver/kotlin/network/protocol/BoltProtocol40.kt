...
import com.neo4j.driver.kotlin.network.protocol.bolt40.error.SecurityErrors
import com.neo4j.driver.kotlin.network.protocol.bolt40.handler.RecordCollector
import com.neo4j.driver.kotlin.network.protocol.bolt40.handler.RequestManager
import com.neo4j.driver.kotlin.network.protocol.bolt40.message.request.connected.HelloMessage
import com.neo4j.driver.kotlin.network.protocol.bolt40.message.request.ready.GoodbyeMessage
import com.neo4j.driver.kotlin.network.protocol.bolt40.message.request.streaming.PullMessage
...
import com.neo4j.driver.kotlin.network.protocol.bolt40.message.response.RunMessage
...
class BoltProtocol40(
    ...
    override fun run(query: String, parameters: Map<String, Any?>): List<Map<String, Any?>> {
        ...
        val pullFuture =
            this.requestManager.sendRequest(PullMessage())

        val runResult = runResultFuture.getUnwrapped()
        if (!runResult.value) {
            // TODO: Retry
        ...
        try {
            val pullResult = pullFuture.getUnwrapped()
    ...
    companion object {
         ...
         */
        val version = ProtocolVersion(4, 3, 3)
    ...
    class Factory : BoltProtocol.Factory {
        ...
        override fun create(ch: Channel): BoltProtocol40 {
            ...
            return BoltProtocol40(ch, requestManager)
        }
    }
}